...
while len(seeds) > 0:
    seeds_tuples.append((seeds[0],seeds[1]))
    seeds = seeds[2:]

print(f"seeds: {seeds}")
print(f"seeds_tuples: {seeds_tuples}")
...
for seed in seeds_tuples:
    seed_init = int(seed[0])
    seed_range = int(seed[1])
    print(f"seed: {seed}")

    for i in range(seed_range):
        current_seed = seed_init + i
        soil = mapper(seed_to_soil, current_seed)
